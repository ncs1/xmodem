...
module XMODEM
  ...
  #
  def self.receive(remote, local, options = nil)
    ...
    when :crc
      XMODEM_CRC_ATTEMPTS.times do |attempt|
        ...
        next unless attempt == XMODEM_CRC_ATTEMPTS - 1
    ...
    loop do
      ...
      if check_ok
        ...
        raise(RXChecksumError, "too many receive errors on block #{block}") if error_count > XMODEM_MAX_ERRORS
      end
    rescue RXTimeout
      error_count += 1
      logger.warn("receiver: timeout error # #{error_count} / max #{XMODEM_MAX_ERRORS}")
      raise(RXTimeout("too many receive errors on block #{block}")) if error_count > XMODEM_MAX_ERRORS
    end
  ...
  def self.send(remote, local)
    ...
    loop do
      ...
      if select([remote], nil, nil, timeout_seconds * XMODEM_MAX_TIMEOUTS).nil?
        unless sent_eot
          raise(RXTimeout, "timeout waiting for input on tx (#{timeout_seconds * XMODEM_MAX_TIMEOUTS} seconds)")
        end
      ...
      elsif (block_number == 1) && (tx_cmd == CRC_MODE)
        mode = :crc
  ...
  def self.checksum(block)
    unless block.length == block_size
      raise(RXChecksumError, "checksum requested of invalid block {size should be #{block_size}, was #{block.length}")
    end
  ...
  def self.receive_getbyte(remote)
    if select([remote], nil, nil, timeout_seconds).nil?
      remote.putc(NAK)
      raise(RXTimeout)
    end
